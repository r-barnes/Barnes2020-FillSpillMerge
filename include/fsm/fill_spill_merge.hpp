...
#pragma omp parallel for collapse(2)
    ...
    while (!q.empty())
    {
      ...
      { // not a pit cell
        ...
        if (wtd(c) > 0)
        {                   // Groundwater can go negative, so it's important to make sure that we are only passing positive water around
        ...
        if (--dependencies(n) == 0)
        { // CHECK this is a hack! Something is wrong with the dependencies matrix, should never go below 0 but it sometimes does.
      ...
      std::unordered_map<dh_label_t, dh_label_t> &jump_table)
  {
...

    {
      ...
      if (lchild != NO_VALUE                                                                                                                         // If I am a leaf, then I can't get water from my children
          && deps.at(lchild).water_vol == deps.at(lchild).dep_vol && deps.at(rchild).water_vol == deps.at(rchild).dep_vol && this_dep.water_vol == 0 // If water_vol>0 then children have already overflowed into parent
      )
      {
    ...
    if (this_dep.water_vol > this_dep.dep_vol)
    {
      ...
      assert(
          this_dep.water_vol == 0 || this_dep.water_vol <= this_dep.dep_vol || (this_dep.lchild == NO_VALUE && this_dep.rchild == NO_VALUE) || (this_dep.lchild != NO_VALUE && this_dep.rchild != NO_VALUE && deps.at(this_dep.lchild).water_vol < this_dep.water_vol && deps.at(this_dep.rchild).water_vol < this_dep.water_vol));
    }
      ...
      double extra_water)
  {
    ...
    {                                                                // Can this depression hold any water?
      ...
      {                                                                                    // Is it enough to hold all the extra water?
      ...
      {                                        // It wasn't enough to hold all the water
